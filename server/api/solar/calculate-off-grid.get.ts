...
import type { OffGridMonthlyItem } from '~/types/off-grid'
...
export default defineEventHandler(async (event) => {
  ...
  const pvYield = pvgis.yearlyProduction // kWh/kWp-yil
  const yearlyConsumption = dailyConsumptionKwh * 365

  // Off-grid icin %30 guvenlik marji
  ...
  const efficiency = yearlyConsumption > 0
    ? Math.min((yearlyProduction / yearlyConsumption) * 100, 999)
    : 0
  const areaM2 = panelCount * panelArea

  // Batarya: 3 gun otonom
  const batterySizeKwh = nighttimeConsumptionKwh * 3

  // Maliyet: panel(10k TL/kWp) + inverter(15k TL/kWp) + batarya(8k TL/kWh) * cati katsayisi
  ...
  // Aylik seri
  const monthlySeries: OffGridMonthlyItem[] = pvgis.monthlySeries.map((item) => ({
    month: item.month,
